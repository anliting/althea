...
export default async(env,decodedPathname)=>{
    /*
        ...
        3: is file, in `${dataDir}/${env.config.pathToUsersFiles}`.
        4: is file, in `${dataDir}/${env.config.pathToPlugins}`.
    */
    let path=decodedPathname.split('/')
    return(path[1]=='plugins'&&3<=path.length?
        await f(`${env.althea._dataDir}/${env.config.pathToPlugins}/${path[2]}/files/${
            path.slice(3).join('/')
        }`,4)
    :
        await f(
        ...
        await f(`${env.althea._dataDir}/${env.config.pathToUsersFiles}${decodedPathname}`,3)
    )||{status:1}
